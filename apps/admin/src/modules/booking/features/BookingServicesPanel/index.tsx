...
import { ServiceTypesTable, ServiceTypesCards } from '../ServiceTypesTable';
    ...
    <>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2, gap: 2 }}>
        <Typography variant="body2" color="text.secondary">
          Каталог процедур для записи клиентов
        </Typography>
        {isMobile ? (
      ...
      {isError && (
        <Typography color="error">Не удалось загрузить услуги</Typography>
      )}
      ...
      <ServiceTypeFormDialog
        ...
        onSubmit={(input) => {
          if (dialog.item) updateMutation.mutate({ id: dialog.item.id, input });
