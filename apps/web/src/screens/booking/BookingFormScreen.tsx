...
import { createBookingRequest, fetchBookingServiceTypes } from '../../api/booking';
...
export default function BookingFormScreen() {
  ...
  const formComplete = Boolean(
    service &&
      ...
      (!collectAge || !ageRequired || (petAge.trim() !== '' && parsedAge !== undefined && !Number.isNaN(parsedAge))),
  );
  ...
  const mutation = useMutation({
    ...
    onError: (err: unknown) => setFormError(getApiErrorMessage(err, 'Не удалось отправить заявку')),
  });

  useEffect(() => {
    if (!servicesQuery.isLoading && service && date && needsTime && !slotTime) {
      navigate(`/booking/new/${serviceTypeId}/date?selectDate=${encodeURIComponent(date)}`, { replace: true });
    }
    ...
    <>
      ...
      <div className={styles.wrapper}>
        ...
        <form className={styles.form} onSubmit={handleSubmit}>
          ...
          {collectAge && (
            <label className={styles.field}>
              <span className={styles.label}>Возраст (лет){ageRequired ? ' *' : ''}</span>
              <input
                className={styles.input}
                inputMode="numeric"
                value={petAge}
                onChange={(e) => setPetAge(e.target.value.replace(/\D/g, '').slice(0, 2))}
              />
              {ageWarning && <p className={styles.ageWarn}>{ageWarning}</p>}
            </label>
          )}
          <button type="submit" className={styles.submit} disabled={!formComplete || mutation.isPending}>
