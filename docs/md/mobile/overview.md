...
- `apps/web`: те же экраны (splash, таббар на телефоне / верхняя шапка на десктопе, контент, auth, ЛК, вопрос врачу).
- PWA: manifest, service worker (статика; API не кэшируется), баннер установки.
- Десктоп ≥900px: верхняя шапка (макет B), на телефоне таббар (макет A).
